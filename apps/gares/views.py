...
class GestionRequiredMixin(LoginRequiredMixin, UserPassesTestMixin):
    """Mixin pour restreindre l'accès à la configuration imprimante (chef de
    gare de la gare concernée, ou accès global)."""

    def test_func(self):
        return self.request.user.has_global_access or self.request.user.is_chef_gare


class LectureGareMixin(LoginRequiredMixin, UserPassesTestMixin):
    """Consultation des gares : ouverte à tout le personnel connecté (lecture
    seule) — l'écriture (créer/modifier/supprimer) est réservée au super admin,
    voir SuperAdminRequiredMixin."""
...
class GareListView(LectureGareMixin, ListView):
    """Liste des gares."""
    model = Gare
...
class GareCreateView(SuperAdminRequiredMixin, CreateView):
    """Créer une nouvelle gare — réservé au super admin."""
    model = Gare
    form_class = GareForm
...
class GareUpdateView(SuperAdminRequiredMixin, UpdateView):
    """Modifier une gare — réservé au super admin."""
    model = Gare
    form_class = GareForm
    template_name = 'gares/gare_form.html'
    success_url = reverse_lazy('gares:gare_list')
